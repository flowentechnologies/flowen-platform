 ...
 * Returns { jobId } immediately — generation is async.
 * Poll GET /api/admin/seedance/[jobId] until status === "succeeded".
 */

import { NextRequest, NextResponse } from 'next/server';
import { requireAdmin } from '@/lib/admin/guard';
import { createVideoTask, type SeedanceModel, type SeedanceRatio, type SeedanceResolution, type SeedanceStyle } from '@/lib/byteplus/seedance';
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    const jobId = await createVideoTask({
      ...
      aspect_ratio:    body.aspect_ratio    as SeedanceRatio | undefined,
