        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-sm font-mono border transition-all ${
            active === i
              ...
              : 'bg-slate-100 dark:bg-slate-800 border-slate-300 dark:border-slate-700 text-slate-600 dark:text-slate-300 hover:border-slate-500'
          }`}
        >
          {word}
        </button>
        ...
        <button
          ...
          className={`w-full text-left px-3 py-2 rounded-lg text-sm border transition-all ${
            active === i
              ...
              : 'bg-slate-800/50 border-slate-700/50 text-slate-600 dark:text-slate-300 hover:border-slate-600'
          }`}
        >
...
function PassageDisplay({ text }: { text: string }) {
  ...
  return (
    <div className="bg-slate-100/60 dark:bg-slate-800/40 rounded-xl px-4 py-4 text-sm text-slate-700 dark:text-slate-200 leading-loose">
      {segments.map((seg, i) => (
        <span key={i}>
          {seg}
          {i < segments.length - 1 && (
...
function PromptCard({ prompt }: { prompt: string }) {
  ...
  return (
    <div className="bg-slate-100/60 dark:bg-slate-800/40 rounded-xl px-4 py-4 space-y-2">
      <p className="text-slate-900 dark:text-white text-sm font-semibold leading-relaxed">{lines}</p>
      {rest.map((para, i) => (
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-6 space-y-4">
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="text-slate-900 dark:text-white font-semibold text-sm mt-0.5 truncate">{ex.title}</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <button
            onClick={() => setIdx(i => Math.max(0, i - 1))}
            disabled={idx === 0}
            className="w-7 h-7 flex items-center justify-center rounded-lg border border-slate-300 dark:border-slate-700 text-slate-400 hover:border-slate-500 hover:text-slate-200 disabled:opacity-30 disabled:cursor-not-allowed transition-all"
      ...
      <p className="text-slate-400 text-xs leading-relaxed italic border-l-2 border-slate-300 dark:border-slate-700 pl-3">
        {ex.instructions}
      </p>
