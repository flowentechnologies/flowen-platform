  ...
  const save = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setSaveErr(json.error ?? 'Failed to save note');
        setSaving(false);
        return;
      }
      setSaved(true);
      ...
      setSaveErr('Network error — check your connection and try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  if (editing) {
    return (
      <div className="flex flex-col gap-1.5 min-w-[180px]">
        ...
        />
        {saveErr && (
          <p className="text-[10px] text-red-400 bg-red-500/10 border border-red-500/20 rounded px-2 py-1">{saveErr}</p>
        )}
        <div className="flex gap-1.5">
          ...
          <button
            onClick={() => { setEditing(false); setText(session.note ?? ''); setSaveErr(null); }}
...
function TreatmentPlanCard({ patientId }: { patientId: string }) {
  ...
  const save = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const json = await res.json().catch(() => ({})) as { error?: string };
        setSaveErr(json.error ?? 'Failed to save plan');
        setSaving(false);
        return;
      }
      ...
      {editing && (
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            ...
            <button type="button" onClick={() => { setEditing(false); setSaveErr(null); }}
          ...
          {saveErr && (
            <p className="text-xs text-red-400 bg-red-500/10 border border-red-500/20 rounded-xl px-4 py-2.5 mt-1">{saveErr}</p>
          )}
        </div>
      )}
    </div>
