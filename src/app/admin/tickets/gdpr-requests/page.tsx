    ...
    <div className="space-y-8">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-6 border-b border-slate-800">
        <div>
          ...
          <h1 className="text-3xl font-extrabold text-white tracking-tight">GDPR Requests</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className={`border rounded-2xl p-5 ${pending > 0 ? 'bg-amber-500/5 border-amber-500/20' : 'bg-slate-900 border-slate-800'}`}>
          <p className="text-[10px] font-mono text-slate-400 uppercase tracking-wide mb-2">Pending</p>
          <p className={`text-4xl font-black ${pending > 0 ? 'text-amber-400' : 'text-white'}`}>{pending}</p>
          <p className="text-[10px] text-slate-600 mt-1 font-mono">awaiting acknowledgement</p>
        </div>
        <div className={`border rounded-2xl p-5 ${overdue > 0 ? 'bg-red-500/5 border-red-500/20' : 'bg-slate-900 border-slate-800'}`}>
          <p className="text-[10px] font-mono text-slate-400 uppercase tracking-wide mb-2">Overdue (30-day SLA)</p>
          <p className={`text-4xl font-black ${overdue > 0 ? 'text-red-400' : 'text-white'}`}>{overdue}</p>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5">
          ...
          <p className="text-4xl font-black text-white">{completedMonth}</p>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl px-5 py-4">
        <p className="text-xs text-slate-400 leading-relaxed font-mono">
          <span className="text-slate-300 font-bold">UK GDPR timelines:</span>{' '}
          Acknowledge within <span className="text-white">72h</span> ·
          Respond within <span className="text-white">30 days</span> (extendable to 90 days for complex requests with written notice) ·
