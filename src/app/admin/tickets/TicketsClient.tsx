...
const PRIORITY_CONFIG: Record<TicketPriority, { label: string; color: string; dot: string }> = {
  ...
  low:    { label: 'Low',    color: 'bg-slate-100 dark:bg-slate-800 text-slate-500',   dot: 'bg-slate-700' },
};
...
function TicketDrawer({ ticket, adminEmail, onUpdate, onClose }: {
  ...
  return (
    <div className="fixed inset-0 z-40 flex items-stretch">
      ...
      <div className="w-full max-w-2xl bg-slate-50 dark:bg-slate-950 border-l border-slate-200 dark:border-slate-800 flex flex-col overflow-hidden">
        {/* Header */}
        <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-start gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              <span className="px-1.5 py-0.5 rounded text-[9px] font-mono font-bold bg-slate-100 dark:bg-slate-800 text-slate-400">{CATEGORY_LABELS[ticket.category]}</span>
          ...
          <button type="button" onClick={onClose} className="text-slate-500 hover:text-slate-900 dark:hover:text-white text-xl shrink-0 mt-0.5">×</button>
        </div>

        {/* Controls */}
        <div className="px-6 py-3 border-b border-slate-200 dark:border-slate-800 flex flex-wrap gap-3">
        ...
        <div ref={threadRef} className="flex-1 overflow-y-auto px-6 py-5 space-y-4">
          ...
          <div className="flex gap-3">
            <div className="w-7 h-7 rounded-full bg-slate-700 flex items-center justify-center text-xs font-bold text-slate-600 dark:text-slate-300 shrink-0">
            ...
            <div className="flex-1">
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 text-sm text-slate-600 dark:text-slate-300 leading-relaxed whitespace-pre-wrap">
                {ticket.body}
              </div>
            </div>
          </div>
          ...
          {ticket.messages.map(msg => (
            <div key={msg.id} className={`flex gap-3 ${msg.from_admin ? 'flex-row-reverse' : ''}`}>
              <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ${msg.from_admin ? 'bg-indigo-600 text-slate-900 dark:text-white' : 'bg-slate-700 text-slate-600 dark:text-slate-300'}`}>
              ...
              <div className={`flex-1 ${msg.from_admin ? 'items-end' : ''}`}>
                ...
                <div className={`rounded-xl px-4 py-3 text-sm leading-relaxed whitespace-pre-wrap ${msg.from_admin ? 'bg-indigo-600/20 border border-indigo-500/30 text-indigo-100' : 'bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-300'}`}>
                  {msg.body}
                </div>
              </div>
            </div>
          ...
          {notesEditing ? (
            <div className="space-y-2">
              ...
              <textarea value={notes} onChange={e => setNotes(e.target.value)} rows={3}
                className="w-full bg-white dark:bg-slate-900 border border-amber-500/30 rounded-lg px-3 py-2 text-xs text-slate-900 dark:text-white font-mono focus:outline-none resize-none" />
              <div className="flex gap-2">
                <button type="button" onClick={saveNotes} disabled={isPending}
                  className="px-3 py-1.5 text-xs font-mono font-bold rounded-lg bg-amber-600 hover:bg-amber-500 text-slate-900 dark:text-white transition-colors disabled:opacity-40">
                  Save Notes
                </button>
...
function TicketRow({ ticket, onClick }: { ticket: Ticket; onClick: () => void }) {
  ...
  return (
    <tr
      className="border-b border-slate-200 dark:border-slate-800/60 last:border-0 hover:bg-slate-800/30 cursor-pointer transition-colors"
      onClick={onClick}
    >
      ...
      <td className="px-3 py-3">
        <p className="text-sm font-medium text-slate-900 dark:text-white truncate max-w-[260px]">{ticket.subject}</p>
        ...
        <div className="flex gap-1">
          {STATUS_TABS.map(t => (
            <button key={t.id} type="button" onClick={() => setTab(t.id)}
              className={`px-3 py-1.5 text-xs font-mono rounded-lg transition-colors ${tab === t.id ? 'bg-indigo-600 text-slate-900 dark:text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-400 hover:text-slate-900 dark:hover:text-white'}`}>
