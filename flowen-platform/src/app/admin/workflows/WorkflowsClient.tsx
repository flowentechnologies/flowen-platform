...
function WorkflowCard({
  ...
  return (
    <div className={`bg-white dark:bg-slate-900 border rounded-2xl overflow-hidden transition-colors ${wf.status === 'active' ? 'border-slate-300 dark:border-slate-700' : 'border-slate-200 dark:border-slate-800'}`}>
      {/* Header row */}
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700 rounded-2xl w-full max-w-md shadow-2xl">
            ...
            <div className="px-6 py-5 space-y-4">
              <div>
                ...
                <input value={form.name} onChange={e => field('name', e.target.value)} placeholder="Workflow name" className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500/60" />
              </div>

              <div>
                <label className="block text-[10px] font-mono text-slate-400 uppercase tracking-wide mb-1.5">Description</label>
                <textarea value={form.description} onChange={e => field('description', e.target.value)} rows={2} placeholder="What does this workflow do?" className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white placeholder-slate-600 focus:outline-none focus:border-indigo-500/60 resize-none" />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-[10px] font-mono text-slate-400 uppercase tracking-wide mb-1.5">Trigger Type</label>
                  <select value={form.trigger_type} onChange={e => field('trigger_type', e.target.value as TriggerType)} className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500/60">
                    {TRIGGER_TYPES.map(t => (
                ...
                <div>
                  ...
                  <select value={form.status} onChange={e => field('status', e.target.value as WorkflowStatus)} className="w-full bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg px-3 py-2 text-sm text-slate-900 dark:text-white focus:outline-none focus:border-indigo-500/60">
