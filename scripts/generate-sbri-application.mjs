...
p(doc, 'Flowen addresses this gap directly: a real-time acoustic biofeedback platform whose on-device audio pipeline runs at sub-80ms latency during self-directed practice, with block/prolongation/repetition detection resolving within 300ms end-to-end — and gives NHS Speech & Language Therapists a way to extend their clinical reach — reviewing session data and adjusting a patient\'s programme remotely — without requiring an in-person appointment for every check-in. Flowen is designed to increase the volume and consistency of practice a person who stammers can access between clinical touchpoints, not to replace the SLT relationship.');
...
p(doc, 'Flowen combines a proprietary, real-time acoustic pipeline — purpose-built to detect the acoustic patterns of stammered speech (blocks, prolongations, repetitions) that general-purpose speech recognition, trained overwhelmingly on fluent speech, is not designed to characterise — with OpenAI\'s Whisper automatic speech recognition (ASR) for transcription, prompt-conditioned for fluency-shaping therapy vocabulary. The acoustic pipeline runs at sub-80ms latency, fast enough to respond to a speaker mid-sentence; transcription is a separate, asynchronous step per utterance and is not part of that real-time loop.');
...
note(doc, 'Company financial position, stated honestly: Flowen is currently pre-funding — cash in bank and monthly burn are both minimal (founder-funded, sweat-equity basis), and no capital has yet been committed toward the £350,000 pre-seed round the company is raising. The "18-month runway" referenced elsewhere in Flowen\'s compliance tracking (DTAC item dtac_10) is the projected runway upon completion of that round, not a current-state fact — that document has been corrected to say so. A real SBRI submission should not imply the round is closed or that an 18-month runway exists today; it should state the current pre-funding position plainly and, if relevant to assessment, note that SBRI Phase 1 funding would itself extend runway independent of the pre-seed round\'s outcome.');
